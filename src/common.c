...
int dis_open(const char* file, int flags)
{
	...
	if((fd = _open(file, flags)) < 0)
	{
		...
		_snprintf(file_truncated, DIS_XOPEN_ARBITRARY_VALUE, "%s", file);
...
ssize_t dis_read(int fd, void* buf, size_t count)
{
	...
	if((res = _read(fd, buf, (unsigned int)count)) < 0)
	{
		dis_errno = errno;
...
ssize_t dis_write(int fd, void* buf, size_t count)
{
	...
	if((res = _write(fd, buf, (unsigned int)count)) < 0)
	{
		dis_errno = errno;
...
off64_t dis_lseek(int fd, off64_t offset, int whence)
{
	...
	if((res = _lseeki64(fd, offset, whence)) < 0)
	{
		dis_errno = errno;
...
void hexdump(DIS_LOGS level, uint8_t* data, size_t data_len)
{
	...
	for(i = 0; i < data_len; i += offset)
	{
		...
		_snprintf(s, 12, "0x%.8zx ", i);
		max = (i+offset > data_len ? data_len : i + offset);

		for(j = i; j < max; j++)
			_snprintf(&s[11 + 3*(j-i)], 4, "%.2x%s", data[j], (j-i == offset/2-1 && j+1 != max) ? "-" : " ");
